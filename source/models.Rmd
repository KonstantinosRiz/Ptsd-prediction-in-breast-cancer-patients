...
new_train_features <- train_features[ , !names(train_features) %in% 
                                    c('Emot_Fun_QLQ30.0', 'Emot_Fun_QLQ30.3', 'nccn_distress_thermometer.0', 'nccn_distress_thermometer.3')]
new_test_features <- test_features[ , !names(test_features) %in% 
                                    c('Emot_Fun_QLQ30.0', 'Emot_Fun_QLQ30.3', 'nccn_distress_thermometer.0', 'nccn_distress_thermometer.3')]

#### This should be commented out if we don't want to remove the above features
...
# ! Temporarily commented out for different approach

# one_hot_train_features <- model.matrix(~ . - 1, data = train_features)
# one_hot_test_features <- model.matrix(~ . - 1, data = test_features)
# yInd <- data.frame(model.matrix.lm(Depression_HADS_clusters ~ ., data = ytrain[,-1], na.action = "na.pass"))[,-1]
...
for (feature in keys(features_dict)) {
  if (features_dict[[feature]] >= div(rfe_iterations, 2)) {
...
for (name in one_hot_chosen_features) {
  if (!(name %in% names(one_hot_train_features))) {
    print(name)
  }
}
...
Run the wanted model using all the combinations of the hyperparameters in the respective grid, find the optimal model and evaluate it on the test set. We try both the full-featured dataset where there are no limitations. We try the one-hot encoded dataset where we can't use factors.
