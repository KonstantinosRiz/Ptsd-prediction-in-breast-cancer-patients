...
metric <- c(rep('auc', no_class), rep('f2', no_class))
...
results_df <- data.frame(classifier, metric, score)
results_df$classifier <- factor(results_df$classifier, levels=original_classifier)

results_plot <- ggplot(
  data = results_df,
  mapping = aes(x = classifier, y = score, fill = metric)
) +
  geom_col(position = 'dodge') +
  scale_fill_manual(values = c('#417FBA', '#F2555E')) +
  ggtitle('Performance using all features')
...
score <- c(auc_scores_reduced, f2_scores_reduced)

# Pretty results df
results_df_reduced_pretty <- data.frame(original_classifier, auc_scores_reduced, f2_scores_reduced)

# Df for visualization
results_reduced_df <- data.frame(classifier, metric, score)
results_reduced_df$classifier <- factor(results_reduced_df$classifier, levels=original_classifier)

results_reduced_plot <- ggplot(
  data = results_reduced_df,
  mapping = aes(x = classifier, y = score, fill = metric)
) +
  geom_col(position = 'dodge') +
  scale_fill_manual(values = c('#417FBA', '#F2555E')) +
  ggtitle('Performance using reduced features')
